...
### Security & Privacy
Password protection / encryption · Security & permissions inspector (read-only) · Manual redaction · Smart PII auto-redaction (worker-based) · Sanitize (strip JS, embedded files, hidden layers) · Flatten annotations into pages
...
Compress PDFs · PDF/A archival conversion · Print preparation (bleed, marks) · Repair corrupted PDFs · Font inspector · Image inspector (read-only) · Page-box / print-geometry inspector (read-only) · Layers / OCG inspector (read-only) · Links inspector · JavaScript & actions inspector (read-only) · Document statistics · Metadata editor
