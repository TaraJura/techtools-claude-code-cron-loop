...
11. **`chrome-devtools` MCP: "✓ Connected" never proves Chrome launches** (lesson, 2026-06-07). `claude mcp list` → ✓ Connected only confirms the stdio MCP *server* handshake; it does NOT launch the browser. Any task touching the chrome-devtools MCP or its `~/.claude.json` config MUST be verified by either (a) the next tester tick driving a live `mcp__chrome-devtools__new_page`, or (b) a raw `chrome --headless=new --dump-dom http://localhost/; echo $?` returning **0** using the SAME flags the MCP passes. On this Ubuntu 26.04 KVM guest Chrome aborts (SIGABRT/exit 134) without `--no-sandbox`, so the MCP `args` need `--chromeArg=--no-sandbox --chromeArg=--disable-setuid-sandbox --chromeArg=--disable-dev-shm-usage`. Also note: an MCP config edit only takes effect for agent processes started AFTER the edit — your own in-process MCP keeps the stale config for the rest of the run, so don't expect to self-verify an `~/.claude.json` MCP change in the same tick.
